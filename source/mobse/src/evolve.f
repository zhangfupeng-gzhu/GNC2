***
      SUBROUTINE evolve(kstar,mass0,mass,rad,lumin,massc,radc,
...
*
* Reference for updates Giacobbo et al. (submitted)
*
***
*
* March 2001 *
...
*
* 6) Note that eq.42 in the paper is missing a SQRT aexit example fortranround the
...
*
      ...
      REAL*8 mass1i,mass2i,tbi,ecci,t_gw
      ...
      integer max_op_line
      parameter(max_op_line=2000)
      REAL bcm(50000,34),bpp(max_op_line,33)
      COMMON /BINARY/ bcm,bpp
*
      LOGICAL saveflag
      COMMON /LOAD/ saveflag
*
*
...
*
     ...
     &                              (rad(1)**2.d0))**(1.d0/4.d0))
                ...
                bpp(jp,12) = epoch(1)
                bpp(jp,13) = ospin(1)
                bpp(jp,14) = dmt(1) - dmr(1)
                bpp(jp,15) = rad(1)/rol(1)
     ...
     &                               (rad(2)**2.d0))**(1.d0/4.d0))
                ...
                bpp(jp,26) = epoch(2)
                bpp(jp,27) = ospin(2)
                bpp(jp,28) = dmt(2) - dmr(2)
                bpp(jp,29) = rad(2)/rol(2)
                bpp(jp,30) = tb
                bpp(jp,31) = sep
                bpp(jp,32) = ecc
                if(bsymb)then
...
*
* Compute the merging time due to gw emission:
*	t_gw = 1/256 * c^5/G^3 * [a^4*(1 - e^2)^(7/2)]/[mass(1)*mass(2)*(m1 + m2)]
* where c = 1.3594037*10^7 [Rsun / yr] 
*       G = 3.9227785*10^8 [Rsun^3 / (Msun * yr^2)]
*
*         t_gw = 5.d0/256.d0*(1.3594037d0*10**7)**5/
*     &       (3.9227785d0*10**8)**3*
*     &	     sep**4*(1 - ecc2)**(7.d0/2.d0)/
*     &       (mass(1)*mass(2)*(mass(1)+mass(2)))/(10**6)
*
* ... but probably it's better to evolve the system in any case
      t_gw = 0.0
* GWs are important only for compact objects
      if((kstar(1).ge.10.and.kstar(2).ge.10).and.t_gw.le.tphysf)then
...
* evolution formulae.
         if(mt.gt.300.d0.and.saveflag.eqv..true.)then
            WRITE(99,*)' MASS EXCEEDED in evolve.f',mass1i,mass2i,
     &                 tbi,ecci,mt
            saveflag = .false.
            goto 150
         endif
...
*
         ...
         bpp(jp,3) = mass0(1)
         bpp(jp,4) = mass(1)
         if(kstar(1).eq.15) bpp(jp,4) = mass0(1)
         bpp(jp,5) = log10(lumin(1))
         bpp(jp,6) = log10(rad(1))
         teff1 = 1000.d0*((1130.d0*lumin(1)/
     &                       (rad(1)**2.d0))**(1.d0/4.d0))
         ...
         bpp(jp,17) = mass0(2)
         bpp(jp,18) = mass(2)
         if(kstar(2).eq.15) bpp(jp,18) = mass0(2)
         bpp(jp,19) = log10(lumin(2))
         bpp(jp,20) = log10(rad(2))
         teff2 = 1000.d0*((1130.d0*lumin(2)/
     &                        (rad(2)**2.d0))**(1.d0/4.d0))
...
*
      ...
      do 90 , k = 1,2
         ...
         mc = massc(k)
*
* MM: Masses over 300Msun should probably not be trusted in the
* evolution formulae.
*
         if(mt.gt.300.d0.and.saveflag.eqv..true.)then
            WRITE(99,*)' MASS EXCEEDED in evolve.f',mass1i,mass2i,
...
*
      ...
      tb = tb*yeardy
      if(jp.ge.max_op_line.and.saveflag.eqv..true.)then
         WRITE(99,*)' EVOLVE ARRAY ERROR ',mass1i,mass2i,tbi,ecci
         saveflag = .false.
         goto 150
      elseif(jp.ge.160)then
         WRITE(99,*)' EVOLVE ARRAY WARNING ',mass1i,mass2i,tbi,ecci,jp
      endif
      bcm(ip+1,1) = -1.0
      bpp(jp+1,1) = -1.0
*	
