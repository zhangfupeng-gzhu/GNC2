...
module md_s1d_hst_ird_basic_type
	use my_intgl
	!use md_sts_fc
	...
	type,extends(type_s1d_ird_basic):: s1d_hst_ird_basic_type
        ...
        contains
        procedure::print=>print_s1_hst_ird
        procedure::get_s1d_ird_hst
        procedure::get_s1d_ird_hst_weight
		procedure::get_s1d_ird_hst_weight_kernel
        generic::get_hst=>get_s1d_ird_hst, get_s1d_ird_hst_weight
	end type
	!interface get_hst
	!	module procedure get_hst_no_weight
	!	module procedure get_hst_weight
	!end interface
    private::print_s1_hst_ird,get_s1d_ird_hst,get_s1d_ird_hst_weight
    
contains
	subroutine init_s1_hst_ird_basic(this, xmin,xmax, n, use_weight)
		implicit none
		class(s1d_hst_ird_basic_type)::this
		integer n
		real(8) xmin,xmax
        ...
        !print*, "1"
		call init_s1d_ird_basic(this, xmin,xmax,n, sts_type_dstr)
        !print*, "2"
    ...
    subroutine print_s1_hst_ird(this,str_,print_y2_in)
        implicit none
        class(s1d_hst_ird_basic_type)::this
        ...
        !print*, "3"
		if(this%use_weight)then
			write(*, fmt="(10A15)") "bin_type", "nbin", "xmin", "xmax", "NS", "NSW"
			write(*, fmt="(A15, I15, 2E15.5, I15, E15.5 )") str_bin_type, this%nbin, &
            this%xmin, this%xmax, this%ns, this%nsw
			write(unit=*, fmt="(20A20)") "X", "FX", "FXW", "NB", "NBW", "XSTEP"
			do i=1, this%nbin
				write(unit=*, fmt="(3E20.10, I20, 2E20.10)") this%xb(i), this%fx(i), this%fxw(i),&
					this%nb(i), this%nbw(i), this%xsteps(i)
			end do
		else
			write(*, fmt="(10A15)") "bin_type", "nbin", "xmin", "xmax", "NS"
			!print*, "2"
			write(*, fmt="(A15, I15, 3E15.5, I15 )") str_bin_type, this%nbin, &
            this%xmin, this%xmax,  this%ns
			write(unit=*, fmt="(20A20)") "X", "FX", "NB", "XSTEP"
			do i=1, this%nbin
				write(unit=*, fmt="(2E20.10, I20, E20.10)") this%xb(i), this%fx(i), this%nb(i), this%xsteps(i)
			end do
		end if
        !print*, "4"
    end subroutine
...
!	!	integer,optional,intent(in):: use_ylog
	...
	subroutine get_s1d_ird_hst_weight(s1_hst,x,w,n)
		implicit none
		class(s1d_hst_ird_basic_type)::s1_hst
		...
		!print*, "x=",x(1:10), n, s1_hst%nbin
		call get_dstr_num_in_each_ird_bin_weight(x(1:n),w,n,s1_hst%xb, s1_hst%xsteps, s1_hst%nbin, &
			s1_hst%nbw, s1_hst%nsw)
		s1_hst%fxw=s1_hst%nbw/s1_hst%xsteps

		call get_dstr_num_in_each_ird_bin(x(1:n),n,s1_hst%xb, s1_hst%xsteps, s1_hst%nbin, &
			s1_hst%nb, s1_hst%ns)
		s1_hst%fx=dble(s1_hst%nb)/s1_hst%xsteps
	end subroutine
	subroutine get_s1d_ird_hst_weight_kernel(s1_hst,x,w,n,kernel_flag)
		implicit none
		class(s1d_hst_ird_basic_type)::s1_hst
		integer kernel_flag
		integer,parameter::kernel_normal=2,kernal_delta=1
		!type(sts_fc_type)::s1d
		integer n
		real(8) x(n),w(n)
		integer i, indx
		...
		case(kernel_normal)
			call get_dstr_num_in_each_ird_bin_weight_normal_kernel(x(1:n),w,n,s1_hst%xmin,s1_hst%xmax,s1_hst%xb, &
			s1_hst%xsteps, s1_hst%nbin, s1_hst%nbw, s1_hst%nsw)
			s1_hst%fxw=s1_hst%nbw/s1_hst%xsteps
			call get_dstr_num_in_each_ird_bin_normal_kernel(x(1:n),n,s1_hst%xmin,s1_hst%xmax,s1_hst%xb, &
			s1_hst%xsteps, s1_hst%nbin, s1_hst%nb, s1_hst%ns)
			s1_hst%fx=s1_hst%nb/s1_hst%xsteps
		case(kernal_delta)
			call get_dstr_num_in_each_ird_bin_weight_delta_kernel(x(1:n),w,n,s1_hst%xmin,s1_hst%xmax,s1_hst%xb, &
			s1_hst%xsteps, s1_hst%nbin, s1_hst%nbw, s1_hst%nsw)
			s1_hst%fxw=s1_hst%nbw/s1_hst%xsteps
			call get_dstr_num_in_each_ird_bin_delta_kernel(x(1:n),n,s1_hst%xmin,s1_hst%xmax,s1_hst%xb, &
			s1_hst%xsteps, s1_hst%nbin, s1_hst%nb, s1_hst%ns)
			s1_hst%fx=s1_hst%nb/s1_hst%xsteps
			!call get_dstr_num_in_each_ird_bin_normal_kernel(x(1:n),n,s1_hst%xb, s1_hst%xsteps, s1_hst%nbin, &
			!	s1_hst%nb, s1_hst%ns)
			!s1_hst%fx=dble(s1_hst%nb)/s1_hst%xsteps
		case default
			print*, "error define kernel type in s1d ird"
			stop
		end select
	end subroutine
	
	subroutine get_s1d_ird_hst(s1_hst,x,n)
		implicit none
		class(s1d_hst_ird_basic_type)::s1_hst
		!type(sts_fc_type)::s1d
		integer n
...
!
	...
	subroutine output_s1d_hst(s1d_hst, fn)
        implicit none
        type(s1d_hst_ird_basic_type)::s1d_hst
        character*(*) fn
        integer i
        open(unit=999,file=trim(adjustl(fn))//"_s1d_hst.txt")
		if(s1d_hst%use_weight)then
			write(unit=999,fmt="(10A27)") "xb", "fx", "fxw", "nb", "nbw", "xsteps"
			do i=1, s1d_hst%nbin
				!if(isnan(s1d%fxw(i)).or.s1d%ns.eq.0)then
				!	print*, "warnning: i, fx, fxw, ns=", i,s1d%fx(i), s1d%fxw(i), s1d%ns
				!end if
				write(unit=999,fmt="(1P3E27.12E4, I27, 2PE27.12E4)") s1d_hst%xb(i), s1d_hst%fx(i), &
					s1d_hst%fxw(i), s1d_hst%nb(i), s1d_hst%nbw(i), s1d_hst%xsteps(i)
			end do
		else
			write(unit=999,fmt="(10A27)") "xb", "fx", "nb", "xsteps"
			do i=1, s1d_hst%nbin
				write(unit=999,fmt="(1P2E27.12E4, I27, 1PE27.12E4)") s1d_hst%xb(i), s1d_hst%fx(i), s1d_hst%nb(i),&
				 	s1d_hst%xsteps(i)
			end do
		end if
		close(999)
...
module md_s1d_hst_ird_type
	use md_s1d_hst_ird_basic_type
	type,extends(s1d_hst_ird_basic_type):: s1d_hst_ird_type
...
contains 
	subroutine init_s1d_hst(this, xmin,xmax, n, use_weight)
		implicit none
		class(s1d_hst_ird_type)::this
		integer n
		real(8) xmin,xmax
		logical,optional:: use_weight
		if(present(use_weight))then
			call init_s1_hst_ird_basic(this,  xmin,xmax,n, use_weight)
		else
			call init_s1_hst_ird_basic(this,  xmin,xmax,n, .false.)
		end if
	...
	subroutine read_s1d_hst(s1d,file_unit, iostat, iomsg)
		implicit none
		class(s1d_hst_ird_type), intent(inout)::s1d
		...
		read(unit=file_unit) s1d%xb(1:n), s1d%fx(1:n), s1d%nb(1:n),s1d%xsteps(1:n), s1d%ns
		if(s1d%use_weight)then
			read(unit=file_unit)  s1d%nbw(1:n), s1d%fxw(1:n), s1d%xsteps(1:n), s1d%nsw
		endif
	end subroutine
	subroutine write_s1d_hst(s1d,file_unit, iostat, iomsg)
		implicit none
		class(s1d_hst_ird_type), intent(in)::s1d
		integer,intent(in):: file_unit
		integer,intent(out)::iostat
		character(*), intent(inout) :: iomsg
		integer n
		...
		n=s1d%nbin
		write(unit=file_unit) s1d%xb(1:n), s1d%fx(1:n), s1d%nb(1:n), s1d%xsteps(1:n), s1d%ns
		if(s1d%use_weight)then
			write(unit=file_unit)  s1d%nbw(1:n), s1d%fxw(1:n), s1d%xsteps(1:n), s1d%nsw
		end if
	end subroutine
	subroutine conv_s1d_ird_hst_int_real_arrays(fc, intarr, realarr, logarr)
		!use com_main_gw
		implicit none
		type(s1d_hst_ird_type)::fc
		integer nint, nreal
		...
		realarr(fc%nbin*4+1:5*fc%nbin)=fc%xsteps(1:fc%nbin)
		if(fc%is_spline_prepared)then
		    realarr(fc%nbin*5+1:6*fc%nbin)=fc%y2(1:fc%nbin)
        end if
		realarr(fc%nbin*6+1:6*fc%nbin+3)=&
				(/fc%xmin,fc%xmax,fc%nsw/)

	end subroutine
	subroutine conv_int_real_arrays_s1d_ird_hst(fc, intarr, realarr,logarr)
		!use com_main_gw
		implicit none
		type(s1d_hst_ird_type)::fc
		integer nint, nreal
		...
		fc%xsteps(1:fc%nbin)=realarr(4*fc%nbin+1:fc%nbin*5)
		if(fc%is_spline_prepared)then
	...
	subroutine save_s1d_hst_hdf5(s1d, group_id,  tablename)
        ...
        if(s1d%use_weight)then
            ...
            htable%field_names=(/"   X","  FX"," FXW", "  nb"," nbw"/)	
