...
C                    IOUT=-1: Supress error message                  
C
...
C ------- STIFFNESS DETECTION
		!print*, "IASTI,NACCPT=", IASTI,NACCPT
...
C --- FAIL EXIT
  76  CONTINUE
      ...
      RETURN
  77  CONTINUE
      IF (IPRINT.GT.0.AND.IOUT.GT.0) WRITE(IPRINT,979)X   
      IF (IPRINT.GT.0.AND.IOUT.GT.0) WRITE(IPRINT,*)
     &' STEP SIZE T0O SMALL, H=',H
      IF (IPRINT.GT.0.AND.IOUT.GT.0) WRITE(IPRINT,*) 'REJECT=', REJECT
      if (REJECT)THEN
         IF (IPRINT.GT.0.AND.IOUT.GT.0) WRITE(IPRINT,*) 
     &'FACC1, SAFE=', FACC1, SAFE
      else
         IF (IPRINT.GT.0.AND.IOUT.GT.0) WRITE(IPRINT,*) 
     &'POSNEG, H, FAC=', POSNEG,   
     &    H, FAC
  ...
  79  CONTINUE
      IF (IPRINT.GT.0.AND.IOUT.GT.0) WRITE(IPRINT,979)X
 979  FORMAT(' EXIT OF DOPRI5 AT X=',E20.10) 
      IDID=2
      RETURN
      END
C
...
C ----  H**IORD * MAX ( NORM (F0), NORM (DER2)) = 0.01
      ...
      HINIT=SIGN(H,POSNEG)  
      IF(H.eq.0)then
        print*, "h, h1, hmax, posneg=", h, h1, hmax, posneg
        print*, "der12, iord", der12, iord
        print*, "dny, dnf=", dny, dnf
      end if
      RETURN
      END 
C
