...
C              IS COMPUTED WITH HELP OF THE FUNCTION HINIT8
C
...
C --- INITIAL PREPARATIONS   
      ...
      HMAX=ABS(HMAX)     
      IORD=8  
      IF (H.EQ.0.D0) H=HINIT8(N,FCN,X,Y,XEND,POSNEG,K1,K2,K3,IORD,
...
C --- FAIL EXIT
  ...
  79  CONTINUE
      IF (IPRINT.GT.0) WRITE(IPRINT,979)X
 979  FORMAT(' EXIT OF DOP853 AT X=',E20.6) 
      IDID=2
      RETURN
      END
C
      FUNCTION HINIT8(N,FCN,X,Y,XEND,POSNEG,F0,F1,Y1,IORD,
     &                       HMAX,ATOL,RTOL,ITOL,RPAR,IPAR)
C ----------------------------------------------------------
