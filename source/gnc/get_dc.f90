...
subroutine dm_get_dc_mpi(dm)
    ...
    interface
		...
		subroutine mb_get_dc_mpi_starpt_gx_regular(mb,spp)
			use com_main_gw
			implicit none
			type(mass_bins),target::mb
			!type(s1d_type)::phi_star
	...
	! else
	! 	print*, "dm_get_dc_mpi, no star potential"
	! 	stop
	! 	do i=1, dm%n
...
subroutine get_coeff_ej_spt_y_gx_regular( xb, yb,n, spp,barge,asymp, &
...
do i=nbg, ned, ntasks
	...
	!call cpu_time(t3)		
	!print*, "i,rid=", i,rid
	call get_sigma0(energyx, fgx_mb, f0)
	do j=1, n
		jum=yb(j)	
		!print*, "j,rid=", j,rid
		call get_coeff_ffuncs_cfs_grid(energyx,jum, fgx_mb,spp, &
...
subroutine get_coeff_ej_spt_y_gx_iregular( xb, yb,n, spp,barge,asymp, &
	...
	!i=n-3;j=n-3
		...
		do j=1, n
			...
			!if(jum>0.96)then
				!print*, e_110(i,j), e_0(i,j),ee(i,j)
				!call get_coeff_ffuncs_cfs_grid_circular(energyx, fgx_mb_ir,spp, &
				!dms%rp%fxy(i,j),f0, e_110(i,j), e_0(i,j),ee(i,j), ej(i,j), jj(i,j), j_rest(i,j), j_111(i,j)) 
				!print*, "2=",e_110(i,j), e_0(i,j),ee(i,j)
				!read(*,*)
			!end if
				!print*, rid, i,j, e_110(i,j), e_0(i,j),ee(i,j), ej(i,j), jj(i,j), j_rest(i,j), j_111(i,j)
				!stop
...
subroutine get_coeff_ej_spt_y_ir( xb, yb,n,spp,barge,asymp, &
	e_110,e_0,ee,j_111,j_rest,jj,ej)
	use com_main_gw
	implicit none
	!type(s1d_type)::phi_star
...
subroutine get_coeff_ffuncs_cfs_grid(ex,jum, fgx,spp, rp,ra,pd,jc,&
	...
	!block
	!	real(8) vr_tmp, phi_star_tmp
	!	call get_phi_star_full_range(fphi,log10(ra),phi_star_tmp)
	!		vr_tmp=(2*(10**phi_star_tmp+mbh_dmless/ra-ex)*ra**2-(jum*jc)**2)/ra**2
	!		if(abs(vr_tmp)>0.01)then
	!			print*, "check out!"
	!			print*, "vr(ra), ra, ex, jum, rc=",vr_tmp, ra, ex,jum
	!			print*, "mbhmass=",mbh,mbh_dmless
	!			print*, "rid=",rid
	!			stop
	!		end if
	!end block
	...
	!!nintn=int(3*10**(-log10(jum)*2d0/3d0+1d0))
	lrp=log10(rp)
	lra=log10(ra)
	!
	...
	case(get_dc_method_fast)
		do i=2, nintn-1
		!call get_ffuncs_series(ex, emax, emin_factor, mbh_dmless, f1%xb(i), ra,rp, spp, fgx, f1%fx(i), 0.5d0)
		!print*, "f1%fx(i)=",f1%fx(i)
	...
	case(get_dc_method_sqr)
		!do i=2, nintn-1
		!	!call get_ffuncs_series(ex, emax, emin_factor, mbh_dmless, f1%xb(i), ra,rp, spp, fgx, f1%fx(i), 0.5d0)
		!	!print*, "f1%fx(i)=",f1%fx(i)
		!		call get_ffuncs_series_fast_sqrt(ex,  mbh_dmless, f1%xb(i), jum, jc, ra,rp, spp, fgx, f1%fx(i), 0.5d0,.false.)
		!		call get_ffuncs_series_fast_sqrt(ex,  mbh_dmless, f2%xb(i), jum, jc, ra,rp, spp, fgx, f2%fx(i), 1.5d0,.false.)
		!	end do
		!	call get_ffuncs_series_fast_sqrt(ex, mbh_dmless, lrp,  jum, jc, ra,rp, spp, fgx, f1%fx(1), 0.5d0,.true.)
		!	call get_ffuncs_series_fast_sqrt(ex, mbh_dmless, lra,  jum, jc, ra,rp, spp, fgx, f1%fx(nintn), 0.5d0,.true.)
		!	call get_ffuncs_series_fast_sqrt(ex, mbh_dmless, lrp,  jum, jc, ra,rp, spp, fgx, f2%fx(1), 1.5d0,.true.)
		!	call get_ffuncs_series_fast_sqrt(ex, mbh_dmless, lra,  jum, jc, ra,rp, spp, fgx, f2%fx(nintn), 1.5d0,.true.)
		print*, "finish the code here"
		stop
	end select
	!print*, "f11,f1n, f21,f2n=",f1%fx(1),f2%fx(nintn),f2%fx(1),f2%fx(nintn)
...
100	if(logx0<logxminmid)then 
	...
	!	print*, "logx0,logx1,xmin,xmax,x=",logx0,logx1,logxmin,logxmax,ex
	!	print*, "a-1,t0,t1=",ratio-1,t0,t1,ratio-10**logx0/ex,ratio-10**logx1/ex
	!	print*, "1=", px*t1**(ipow+1),1d0/(ipow+1)*(line_slope*ratio*ex+line_c)*(ratio-10**logx0/ex)**(ipow+1)/(ratio-1)**ipow
	!	print*, "2=", px*t0**(ipow+1),1d0/(ipow+1)*(line_slope*ratio*ex+line_c)*(ratio-10**logx1/ex)**(ipow+1)/(ratio-1)**ipow
	!	print*, "3=", qx*t1**(ipow+2),1d0/(ipow+2)*line_slope*ex*(ratio-10**logx0/ex)**(ipow+2)/(ratio-1)**ipow
	!	print*, "4=", qx*t0**(ipow+2),1d0/(ipow+2)*line_slope*ex*(ratio-10**logx1/ex)**(ipow+2)/(ratio-1)**ipow
	!if(ratio-1<1d-6)then
	!	read(*,*)
	!end if
	if(dint<0)then
		dint=0
	...
	!print*, "res_tmp=", res_tmp, px*(tmax**(ipow+1)-tmin**(ipow+1))-qx*(tmax**(ipow+2)-tmin**(ipow+2))
	!block
	!	real(8) term1,term2,term3
	!	term1=1d0/(ipow+1)*ratio**2*ex**2*(ratio-1)
	!	term2=1d0/(ipow+3)*(ratio-1)**3*ex**2
	!	term3=-2d0/(ipow+2)*(ratio-1)**2*ratio*ex**2
	!print*, "res_tmp=", res_tmp, term1*tmax**(ipow+1)+term2*tmax**(ipow+3)+term3*tmax**(ipow+2)-&
	!		(term1*tmin**(ipow+1)+term2*tmin**(ipow+3)+term3*tmin**(ipow+2))
	!!read(*,*)
	!end block
...
subroutine mb_get_dc_mpi_starpt_gx_regular(mb,spp)
	...
	type(star_pot_para)::spp
	interface 
		subroutine get_coeff_ej_spt_y_gx_regular(xb, yb, n, spp,barge,asymp, e_110,e_0,ee,j_111,j_reset,jj,ej)
			use com_main_gw
			implicit none
			!type(s1d_type)::phi_star
			type(star_pot_para)::spp
	...
	case(dejmodel_EJ)
		...
		call get_coeff_ej_spt_y_gx_regular(xcenter, ycenter,n,spp,&
				common_gx,mb%all%asymp, e_110,e_0,ee,j_111,j_rest,jj,ej)
