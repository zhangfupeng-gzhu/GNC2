...
## Key Features
  ...
  * **Self-consistent Stellar Potential:** The stellar potential of stars is solved after each iteration of simulation according to the Poisson equation.
  ...
  * **Massive Black Hole Mass Growth:** Includes the mass accretion of the MBH from loss-cone accretion (e.g., tidal disruption of stars, direct swallowing of stellar objects), stellar evolution and extreme mass ratio inspirals (EMRIs). 
  ...
  * **Integration of MOBSE Stellar Evolution Code:** The mass, stellar type, and physical radius of the stellar objects in the simulation are automatically updated according to MOBSE, an upgraded version of the BSE code ( Hurley, J. R., Pols, O. R., & Tout, C. A. 2000, MNRAS, 315, 543; Hurley, J. R., Tout, C. A., & Pols, O. R. 2002, MNRAS, 329, 897 ): 
...
* Open MPI can be installed via: apt install openmpi-bin libopenmpi-dev 
...
loadGNC() {
    ...
    if [ -f "$script_dir/set_env.sh" ]; then
        ...
        echo "error：cannot find $script_dir/set_env.sh"
    fi
}
...
3. *Start a new shell* and run the following commands in shell :
```shell
> loadGNC
```
...
* ``M1`` is a cluster consisting of $1$ solar mass stars. The density follows a Dehnen profile, with a total mass of $4\times10^7$ solar masses, $r_a=2.17$ pc and $\gamma=1$. The spin effect of MBH is ignored. The mass of MBH $=10^7$ solar masses and always remains fixed. In this example, there is **No** stellar evolution and mass accretion.
The simulation usually finishes within 1 hour when running with 4 threads. After that, you can plot the evolution of the density profile, tidal disruption event rates, effective radius of the cluster, and the influence radius of the MBH in ``plot'' by running:
```
> loadGNC
> python3 plot.py
``` 

* ``M2_sp`` is a cluster consisting of two mass components (stars + a number fraction of 0.001 stellar mass black holes). The density follows a Dehnen profile, with a total mass of $4\times10^7$ solar masses, $r_a=2.17$ pc and $\gamma=1$. The spin of MBH =1, the mass of MBH $=4\times10^6$ solar masses and always remains fixed. In this example, there is **No** stellar evolution and mass accretion.
...
* ``MKG82_sp_stacc_0.06`` is a cluster consisting of 12 mass components ranging from 0.01 to 150 solar masses. Initially all particles are zero-age main-sequence stars and follow the Kroupa initial mass function. The density follows a Dehnen profile, with a total mass of $7\times10^7$ solar masses, $r_a=1.5$ pc and $\gamma=1$. The spin of MBH =1, the initial mass of MBH is $=10^4$ solar masses. Stellar evolution and mass accretion of MBH are **included**. The simulation usually takes about 18 hours when running with 24 threads. After that, you can plot the evolution of the MBH mass growth, tidal disruption and EMRI event rates in ``plot'' by running:
```
> loadGNC
> python3 evl.py
``` 
...
When running ``ini`` or ``main``, you need to have a parameter file ``model.in`` in the folder, which contains the custom parameters for the simulation; There is a list of all default parameters in ``source/main/model_default.in``. Parameters set in ``model.in`` will overwrite the default parameters. A simple example of ``model.in`` is shown in ``examples/M2_sp``, as follows: 
```
##================================================================
...
## Sets Initial Values of the Cluster's Parameters

In the same folder, ``mfrac.in`` sets the initial values of parameters of cluster's mass components and density profiles. An example of a ``Dehnen`` cluster with two discrete mass components (stars+ stellar mass black holes) is:
```
#number of mass bin 
2  GIVEN  
###########
...
#----------------------------------------------
# the following line sets a 10^-3 fraction of stellar black holes relative to stars
1    0.001   
##########################################
density model = Dehnen
unit= MsunPc
#mtot, in units of msun
mtot=4e7
#ra, in units of pc
ra=2.17
gamma=1.0
...
**Setting `clone_factor`**
The clone factor sets the amplification of clone particles at higher energies. Due to the mass segregation, the following values are recommended:
...
**``particle_weighting_factor`` and ``clone_factor`` should be set, so that in each mass bin, the total number of particles per thread is around 1000~100000**. Depending on your machine, the total number of particles across all mass bins should be smaller than $2\times10^6$, unless you have more than 32 GB of memory. For example, when running M2_sp, you will get lines of output like the following:
```
...
The second example of cluster is a cluster consisting of 12 mass bins, following Kroupa Initial mass function:
```
#number of mass bin   
12  KROUPA
